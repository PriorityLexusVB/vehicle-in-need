...
describe("signInWithPopupCOOPSafe (deprecated)", () => {
  ...
  it("always returns auth/invalid-config error (deprecated)", async () => {
  ...
  it("logs deprecation warning", async () => {
    ...
    await signInWithPopupCOOPSafe(mockAuthWithConfig, mockGoogleProvider);

    expect(consoleSpy).toHaveBeenCalled();
    expect(consoleSpy.mock.calls.some(call => 
      call.some(arg => typeof arg === "string" && arg.includes("deprecated"))
    )).toBe(true);

    consoleSpy.mockRestore();
  });

  it("returns error regardless of Firebase config", async () => {
