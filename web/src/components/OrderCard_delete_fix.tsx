...
export const DeleteErrorMessage: React.FC<DeleteErrorMessageProps> = ({
  error,
  onDismiss,
}) => (
  <div
...
export function useOrderDelete(
  options: UseOrderDeleteOptions = {}
): UseOrderDeleteReturn {
  const {
  ...
  const deleteOrder = useCallback(
    async (orderId: string): Promise<DeleteResult> => {
      ...
      console.log(
        `[OrderDelete] Starting delete request for order: ${orderId}`
      );
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 403 || response.status === 401) {
            userMessage =
              "You do not have permission to delete this order. Please contact your manager.";
...
export const SafeDeleteButton: React.FC<SafeDeleteButtonProps> = ({
  ...
  const { deleteOrder, isDeleting, deletingOrderId, error, clearError } =
    useOrderDelete({
      getAuthToken,
      onSuccess: onDeleted,
    });
