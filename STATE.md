...
- 🟡 **DEPLOY PIPELINE: auth FIXED, ONE IAM grant remaining — `main` HEAD code still NOT live until Rob does a 2-min GCP IAM grant.** Full diagnosis 2026-06-05 (WORK), 3-layer root cause:
  ...
  3. **🔴 REMAINING — ONE IAM BINDING (Rob, ~10 sec, GCP only). CONFIRMED via live build 2026-06-05:** the `cloud-build-deployer` SA key is now in `GCP_CREDENTIALS` (vaulted) and AUTHENTICATES correctly, but the build still fails: `PERMISSION_DENIED: caller does not have permission to act as service account ...109495422803475379349 ... authenticated as cloud-build-deployer@...`. So `cloud-build-deployer` does NOT currently hold `roles/iam.serviceAccountUser` (actAs) on the runtime SA `pre-order-dealer-exchange-860` — despite `CLOUD_BUILD_ERROR_FIX.md` claiming it was granted, the LIVE IAM does not have it (removed or never applied). Claude cannot grant IAM (the deployer key is not an IAM admin). **FIX (GCP Cloud Shell, 10 sec):** `gcloud iam service-accounts add-iam-policy-binding pre-order-dealer-exchange-860@gen-lang-client-0615287333.iam.gserviceaccount.com --member="serviceAccount:cloud-build-deployer@gen-lang-client-0615287333.iam.gserviceaccount.com" --role="roles/iam.serviceAccountUser" --project=gen-lang-client-0615287333` — OR console: IAM → Service Accounts → pre-order-dealer-exchange-860 → PERMISSIONS → GRANT ACCESS → cloud-build-deployer as **Service Account User**. Permanent (IAM bindings don't expire). After it lands, re-run the Cloud Build workflow (`gh run rerun <id>`) → K8 + K3 deploy.
