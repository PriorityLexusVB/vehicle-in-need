...
- 🟡 **DEPLOY PIPELINE: auth FIXED, ONE IAM grant remaining — `main` HEAD code still NOT live until Rob does a 2-min GCP IAM grant.** Full diagnosis 2026-06-05 (WORK), 3-layer root cause:
  ...
  3. **🔴 REMAINING (Rob, GCP console, ~2 min):** the `GCP_CREDENTIALS` service account lacks `roles/iam.serviceAccountUser` (actAs) on the Cloud Build service account `109495422803475379349`. Build submit fails: `PERMISSION_DENIED: caller does not have permission to act as service account projects/gen-lang-client-0615287333/serviceAccounts/109495422803475379349`. **FIX — pick ONE:** (A) GCP Console → IAM & Admin → Service Accounts → find SA `109495422803475379349` → Permissions → Grant Access → add the `GCP_CREDENTIALS` SA email as **Service Account User** (`roles/iam.serviceAccountUser`); OR (B) re-create the Workload Identity pool/provider so the original WIF path works again (more involved); OR (C) grant the `GCP_CREDENTIALS` SA project-level `roles/iam.serviceAccountUser`. Option A is the surgical fix. After the grant, just re-run the failed workflow (`gh run rerun <id>`) or push any commit — K8 + K3 will deploy. GCP project: `gen-lang-client-0615287333`. The `GCP_CREDENTIALS` SA email is visible in GCP Console IAM (it's the SA whose key is in that GitHub secret).
