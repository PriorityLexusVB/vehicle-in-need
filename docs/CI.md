...
This document describes how to configure the required GitHub repository secrets for the Build-and-Push Container workflow (`.github/workflows/build-and-deploy.yml`) which uses GCP Workload Identity Federation for authentication.
...
The numeric GCP project number. This is optional but recommended if you need to reference it elsewhere.
...
1. `GCP_WORKLOAD_IDENTITY_PROVIDER` secret is present
2. `GCP_WORKLOAD_IDENTITY_PROVIDER` matches the expected format (`projects/PROJECT_NUMBER/locations/global/workloadIdentityPools/POOL/providers/PROVIDER`)
3. `GCP_SERVICE_ACCOUNT` secret is present

If validation fails, the workflow will exit early with a descriptive error message, making it easy to diagnose and fix configuration issues.

## Troubleshooting

### Error: "invalid_target" when requesting federated token
...
If the workflow fails with "Missing secret GCP_WORKLOAD_IDENTITY_PROVIDER" or "Missing secret GCP_SERVICE_ACCOUNT":

**Solution:** Add the required secrets to your GitHub repository settings.
